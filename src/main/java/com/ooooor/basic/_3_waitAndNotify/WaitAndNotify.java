package com.ooooor.basic._3_waitAndNotify;

/**
 * 1. wait/notify/notifyAll 必须包含在synchronized代码块内,获取对象监视器
 *      1.1. wait不会平白无故发生,需要条件,条件可能被其他线程修改;
 *      1.2. wait/notify涉及线程间通信,保证原子性;
 * 2. notify()唤醒等待队列, 不释放锁,随机不公平唤醒
 * 3. wait()后等待并释放锁, sleep()不释放任何资源
 * 4. wait()被唤醒,还需再等待重新获得锁,才能执行
 */
public class WaitAndNotify {
    ...
    public static void main(String[] args) {
        ...
        // 需要持有监视器锁，报错：java.lang.IllegalMonitorStateException
        new Thread(() -> {
            try {
                object.wait();
            } catch (InterruptedException e) {
